...
export function useWeek(anchor: DateStr) {
  ...
  return useLiveQuery(async () => {
    const [settingsRow, storedBlocks, storedShifts, rules, exceptions, plan] = await Promise.all([
    ...
    const days = dates.map((date) => {
      ...
      return {
        ...
        blocks: dayBlocks,
        shifts: dayShifts,
        insight: dayInsight(date, dayBlocks, dayShifts, plan, settings),
      }
    })

    return { key, dates, days, blocks, evieBusy, balance, warnings, plan, settings, rules }
  }, [anchor])
}
